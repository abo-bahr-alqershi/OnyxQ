...
namespace ERP_Pro.Application.Features.SettingsParameters.Commands.UpdateCompanyInfo
{
    /// <summary>
    /// أمر تحديث معلومات الشركة
    /// </summary>
    public class UpdateCompanyInfoCommand : IRequest
    {
    ...
    /// <summary>
    /// معالج أمر تحديث معلومات الشركة
    /// </summary>
    public class UpdateCompanyInfoCommandHandler : IRequestHandler<UpdateCompanyInfoCommand>
    {
        private readonly IApplicationDbContext _context;
        private readonly ICurrentUserService _currentUserService;
        private readonly IDateTime _dateTime;

        public UpdateCompanyInfoCommandHandler(
        ...
        public async Task<Unit> Handle(UpdateCompanyInfoCommand request, CancellationToken cancellationToken)
        {
            var entity = await _context.CompanyInfo.FindAsync(request.Id);

            if (entity == null)
            {
                throw new NotFoundException(nameof(CompanyInfo), request.Id);
            }
            ...
            // تحديث شعار الشركة إذا كان هناك تغيير
            if (!string.IsNullOrEmpty(request.LogoPath))
            {
                entity.UpdateLogo(request.LogoPath);
            }
