...
{
    /// <summary>
    /// معالج استعلام الحصول على السنة المالية الافتراضية
    /// </summary>
    public class GetDefaultFiscalYearQueryHandler : IRequestHandler<GetDefaultFiscalYearQuery, FiscalYearDto>
    {
        private readonly IFiscalYearRepository _fiscalYearRepository;

        public GetDefaultFiscalYearQueryHandler(IFiscalYearRepository fiscalYearRepository)
        {
            _fiscalYearRepository = fiscalYearRepository;
        }

        public async Task<FiscalYearDto> Handle(GetDefaultFiscalYearQuery request, CancellationToken cancellationToken)
        {
            var fiscalYear = await _fiscalYearRepository.GetDefaultAsync(request.IncludePeriods);
